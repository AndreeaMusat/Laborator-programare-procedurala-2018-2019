	...
	while (1) {
		...
		if (strcmp(ang.cnp, cnp) == 0) {
			...
			printf("Angajatul cu CNP-ul %s este: %s, %d ani, salariu %lf\n",
				cnp, ang.nume, ang.varsta, ang.salariu);
			break;
		}
	}
...
void adaugare_angajat() {
	...
	if (fout == NULL) {
		exit_message(-1, "Nu am putut deschide fisierul binar pentru adaugare angajat");
	}
...
void scriere_angajati_txt() {
	...
	if (fin == NULL || fout == NULL) {
		exit_message(-1, "Nu am putut deschide unul dintre fisiere pentru scriere txt");
	}
...
void stergere_angajat(char cnp[LEN_CNP]) {
	...
	while (1) {
		...
		if (strcmp(cnp, ang.cnp) == 0) continue;
